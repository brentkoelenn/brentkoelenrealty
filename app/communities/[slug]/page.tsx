...
export default async function CommunityPage({ params }: Props) {
  ...
  const listings = await safeListingCall(() => listingService.getListingsByCity(community.name), []);
      ...
      <Container>
        ...
        <div className="mt-14">
          <h2 className="font-display text-2xl text-ink mb-6">
            {isUsingSampleData ? "Sample Listings" : "Listings"} in {community.name}
          </h2>
          {listings.length > 0 ? (
            ...
            <p className="text-sm text-stone">
              No listings are currently tagged to {community.name}. Use the search page to browse
